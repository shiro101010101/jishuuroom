...
export default function LoginClient({ error }: { error?: string }) {
  ...
  async function loginWith(provider: 'google' | 'line') {
    ...
    await signIn(provider, {
      callbackUrl: '/room/00000000-0000-0000-0000-000000000001',
    })
  }
    ...
    <div className={styles.wrap}>
      <div className={styles.card}>
        <div className={styles.logo}>
          ...
          <h1 className={styles.logoText}>自習室 JP</h1>
        </div>
        <p className={styles.tagline}>みんなと一緒に集中して勉強しよう</p>
        <div className={styles.features}>
          <div className={styles.feature}><span>⏱</span> ポモドーロタイマー</div>
          <div className={styles.feature}><span>🎥</span> カメラで一緒に勉強</div>
          <div className={styles.feature}><span>💬</span> フレンドとチャット</div>
          <div className={styles.feature}><span>✅</span> タスク共有機能</div>
        </div>
        {error && (
        ...
        <div className={styles.divider}><span>ログインして始める</span></div>
        ...
        <p className={styles.notice}>
          ログインすることで利用規約とプライバシーポリシーに同意したことになります。<br />
          荒らし行為・迷惑行為はアカウントBANの対象となります。
        </p>
      </div>
    </div>
  )
